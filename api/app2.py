...
def fetch_product_data(query: str) -> List[Dict[str, Any]]:
    ...
    sources = [
        {"engine": "amazon_search", "q": query, "amazon_domain": "amazon.in", "api_key": API},
        {"engine": "google_shopping", "q": query, "gl": "in", "hl": "en", "location": "California,United States", "api_key": API},
    ]
    ...
    for params in sources:
        try:
            ...
            product_data += response.json().get("organic_results", []) + response.json().get("shopping_results", [])
...
def vectorize_and_search(query: str, data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        vectorlake_id = vectorlake.create({"vectorlake_name": query}).get("vectorlake_id")
        ...
        return vectorlake.search({"vector": search_vector, "vectorlake_id": vectorlake_id, "vector_type": "text", "top_k": 20})
    except Exception:
...
def calculate_price_statistics(product_data: List[Dict[str, Any]]) -> Optional[Dict[str, float]]:
